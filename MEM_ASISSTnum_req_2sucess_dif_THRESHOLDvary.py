                ...
                if sim_time >= arrival and request["id"] not in processed_ids:
                    ...
                    try:
                        ...
                        if wait_time <= tau_mem and F_final >= 0.8:
                           ...
                           if diff > threshold or F_final < 0.8 or completion_time>min_tau_value:  # kritik şart
                             status = "dropped"
                           else:
                             status = "success"
                        else:
...
np.save("Threshold_DIF_0_1_sec.npy", average_diff)
np.save("Threshold_SR_0_1_SEC.npy", average_success_rates)
#np.save("SIMTIMEaverage_diff_memory_assisted_tau_meM_1_mili_succ.npy", average_diff_success)


# Grafik çizimi
plt.figure(figsize=(10, 6))
plt.plot(threshold_list , average_diff_success, label="Mean Time Deviation", marker='o', linewidth=2)
...
# Grafik çizimi
plt.figure(figsize=(10, 6))
plt.plot( threshold_list , average_success_rates, label="success rate", marker='o', linewidth=2)
plt.xlabel("Number of Requests", fontsize=12)
...
plt.plot(threshold_list, average_diff, label="All Requests")
...
plt.plot(threshold_list, average_diff_success, label="Successful Only", color='orange')
plt.xlabel("Number of Requests")
plt.ylabel("Avg. Time Difference (s)")
