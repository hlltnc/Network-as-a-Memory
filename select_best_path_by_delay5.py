...
def select_best_path_by_delay(G, src, dst, path_delay, fidelities, kappa, c_fiber=2e8, max_paths=10, delay_tolerance=1):
    ...
    for path in all_paths[:max_paths]:
        try:
            ...
            # 1. deneme
            if abs(total_path_time - path_delay) <= delay_tolerance:
                return {
                    "path": [path],
            ...
            # 2. deneme – Aynı qubitler döndürülerek tekrar iletim
            transmission_time2 = transmission_time * 2
            total_path_time2 = ent_time + transmission_time2
            F_final2 = calculate_final_fidelity(min_fidelity, kappa, total_path_time2)

            if abs(total_path_time2 - path_delay) <= delay_tolerance:
                return {
                    "path": [path],
                    "distance_km": total_distance_km,
                    "ent_time": ent_time,
                    "transmission_time": transmission_time2,
                    "total_time": total_path_time2,
                    "F_dist": min_fidelity,
                    "F_final": F_final2,
                    "delay_error": abs(total_path_time2 - path_delay),
                    "status": "success"
                }

            # kaydet ama success değil
            path_infos.append({
                "path": [path],
                "distance_km": total_distance_km,
                "ent_time": ent_time,
    ...
    # 3. seçenek: Path kombinasyonları
    combo_infos = []
    for path1, path2 in combinations(all_paths[:max_paths], 2):
        try:
            ...
            if abs(total_time - path_delay) <= delay_tolerance:
                return {
                    "path": [path1, path2],
